...
def log_prob(x):
    ...
    if not np.isfinite(prior_val):
        return -np.inf
    return prior_val + calc_log_likelihood(Vsys,
                                          Kp,
                                          scale,
                                          wl_cube_model,
                                          fTemp,
...
def prior(x):
    ...
    if 146. < Kp < 246. and -50 < Vsys < 50 and -1 < log_scale < 1:
        return 0
    return -np.inf


def sample(nchains, nsample, A_noplanet, fTemp, do_pca=True, best_kp=192.06, best_vsys=0.0, best_log_scale=0.0):
    """
    ...
    pos = np.array([best_kp, best_vsys, best_log_scale]) + 1e-2 * np.random.randn(nchains, 3)
...
if __name__ == "__main__":
    ...
    blaze, NPC, star, SNR, telluric = param_dict['blaze'], param_dict['n_princ_comp'], \
                                      param_dict['star'], param_dict['SNR'], param_dict['telluric']

    lls, ccfs = np.zeros((50, 50)), np.zeros((50, 50))

    # redoing the grid. how close does PCA get to a tellurics-free signal detection?
    A_noplanet, fTemp, fTemp_nopca, just_tellurics = make_data(1.,
                                                               wl_cube_model,
                                                               do_pca=True,
                                                               blaze=blaze,
                                                               n_princ_comp=NPC,
