...
def calc_ccf(model_flux, data_arr_slice, n_pixel):
    ...
    """
    model_vector = jnp.subtract(model_flux, jnp.vstack(jnp.mean(model_flux, axis=1)))  # normalized and such
    ...
    logl = (-0.5 * n_pixel * jnp.log(variance_data + variance_model - 2.0 * cross_variance)).sum()
